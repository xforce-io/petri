...
14. **Real ground-truth content** — petri cannot tell whether the value in a gate artifact was computed by an external tool (test runner, backtest CLI, training script, build, CI) or fabricated by the agent in-prompt. So when the loop's \`until\` gate is on a real metric (numeric comparator, e.g. \`oos_annual_return > 0.09\`, \`tests_passed: true\`, \`loss < 0.1\`), the role producing that artifact MUST invoke an external tool and capture its real output into the artifact JSON. Self-grading where the agent synthesizes the metric value from the hypothesis or mental model is the anti-pattern this rule blocks. Path stays per rule 3 (\`{stage}/{role}/<file>.json\`); the discipline is on HOW the file is populated, not where it lives. When the external tool produces a canonical file elsewhere in the project (e.g. \`data/backtest_results/<run_id>/metrics.json\`), the role's skill should instruct the agent to run that tool, read its output, and write the verified values into the gate's \`{stage}/{role}/<file>.json\`.
