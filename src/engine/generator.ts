...
14. **External ground-truth gates** — petri cannot tell whether a role's artifact contains real work or fabricated numbers. So whenever the loop's work has an external verifier (test runner, build, backtest CLI, training loss output, CI), the \`repeat.until\` gate's \`evidence.path\` should point to a file produced by that EXTERNAL system, NOT to a JSON written by the role itself. Examples: \`evidence.path: "test_output.txt"\` (pytest output), \`evidence.path: "data/backtest_results/{run_id}/oos_metrics.json"\` (backtest CLI output), \`evidence.path: "build/error_count.txt"\` (compiler output). Self-graded gates where one role writes both the artifact and the evidence are a last resort — they orchestrate but do not verify.
